...
double w1[hidenode][outnode];//隐藏层到输出层的权值
double b1[hidenode];//隐藏层阈值
...
int main()
{
	...
	printf("开始训练\n");
	while (trainNum < 1000)    //trainNum为样本训练次数
	{
		e = 0.0;                 //e为误差
	trainNum++;
		train(trainData, outData);      //BP训练
	printf("训练第%d次， 错误率=%8.4lf\n", trainNum, error);
	}
	printf("\n训练完成\n测试数据如下:\n");
